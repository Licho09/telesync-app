#!/usr/bin/env python3
"""
Real Telegram Monitoring Service for TeleSync
Monitors channels and downloads videos automatically
"""
...
class TelegramMonitor:
    def __init__(self, user_id: str, api_id: int, api_hash: str, phone: str, download_path: str = "downloads"):
        """
        Initialize Telegram monitor for a specific user.
        ...
        # Download path will be fetched from web app settings
    ...
    def setup_event_handlers(self):
        ...
        async def handle_new_message(event):
            ...
            try:
                ...
                # Get channel ID for matching
                channel_id = getattr(chat, 'id', None)
                channel_id_str = str(channel_id) if channel_id else None
                
                # Check multiple possible identifiers including channel ID
                is_monitored = (
                    ...
                    # Also check if any monitored channel contains this channel's ID
                ...
                if not is_monitored:
                    ...
                    logger.info(f"Channel identifiers checked: {channel_identifier}, {channel_name}, {channel_url}, {channel_id_str}")
                    return
                
                logger.info(f"Processing message from monitored channel: {channel_name} (@{channel_username})")
                
                # Check if message contains video (direct file or URL)
    ...
    def is_video_url(self, text):
        """Check if text contains a video URL (YouTube, etc.)."""
        if not text:
            return False, None
    ...
    async def download_video_from_url(self, url, channel_name, channel_username):
        ...
        try:
            ...
            logger.info(f"Current download path: {self.download_path}")
            ...
            if process.returncode == 0:
                ...
                if downloaded_files:
                    file_path = downloaded_files[0]  # Get the first (and likely only) file
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if settings_data.get('success'):
                    ...
                    self.base_download_path = settings.get('downloadPath', 'discord_downloads')
                    
                    # Handle absolute vs relative paths
                    if os.path.isabs(self.base_download_path):
                        # Absolute path - use as is
                        self.download_path = Path(self.base_download_path) / "telegram" / self.user_id
                    else:
                        # Relative path - make it absolute from Downloads folder (more user-friendly)
                        downloads_folder = Path.home() / "Downloads"
                        self.download_path = downloads_folder / self.base_download_path / "telegram" / self.user_id
            ...
            # Fallback to default path in Downloads folder
            self.base_download_path = "discord_downloads"
            downloads_folder = Path.home() / "Downloads"
            self.download_path = downloads_folder / self.base_download_path / "telegram" / self.user_id
            self.download_path.mkdir(parents=True, exist_ok=True)
                ...
                if channels_data.get('success'):
                    ...
                    for channel in channels:
                        if channel.get('isActive', False):
                            ...
                            # Add known channel IDs for matching
                            # This is a temporary fix - in production, the web app should resolve channel IDs
                            if '+avKdNDQy7wgwMDJh' in url:
                                self.monitored_channels.append('2510784725')  # Your channel ID
    ...
    async def start(self):
        ...
        logger.info(f"Starting Telegram monitor for user {self.user_id}")
        ...
        try:
            ...
            logger.error(f"Error starting Telegram monitor for user {self.user_id}: {e}")
            raise
    
    async def stop(self):
        """Stop the Telegram monitor."""
        logger.info(f"Stopping Telegram monitor for user {self.user_id}")
        await self.client.disconnect()

# Global monitor instances
monitors: Dict[str, TelegramMonitor] = {}

async def start_monitor(user_id: str, api_id: int, api_hash: str, phone: str, channels: List[str] = None):
    """Start monitoring for a specific user."""
    try:
        # Create monitor instance
        monitor = TelegramMonitor(user_id, api_id, api_hash, phone)
        
        # Add channels if provided
        if channels:
            for channel in channels:
                await monitor.add_channel(channel)
...
async def stop_monitor(user_id: str):
    """Stop monitoring for a specific user."""
    if user_id in monitors:
        await monitors[user_id].stop()
        del monitors[user_id]
        logger.info(f"Stopped monitor for user {user_id}")

# Example usage
async def main():
    # This will be called by the Node.js backend
    import sys
    
    if len(sys.argv) < 5:
        print("Usage: python telegram_monitor.py <user_id> <api_id> <api_hash> <phone> [channels...]")
        sys.exit(1)
